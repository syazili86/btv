...
export const actions = {
  async queryItems ({ commit, state }) {
    try {
      ...
      const { status, data } = await this.$axios.get(`/api/program-contents${queryParams}`)
  ...
  async deleteItem ({ dispatch }, { payload, key }) {
    ...
    try {
      ...
      const { status, data } = await this.$axios.delete(`/api/program-contents/${params}`)
  ...
  async addItem ({ dispatch }, { payload }) {
    try {
      const { status, data } = await this.$axios.post(`/api/program-contents`, payload)
  ...
  async updateItem ({ dispatch }, { payload, id }) {
    try {
      const { status, data } = await this.$axios.put(`/api/program-contents/${id}`, payload)
