...
export const actions = {
  async queryItems ({ commit, state }) {
    try {
      ...
      const { status, data } = await this.$axios.get(`/api/programs${queryParams}`)
  ...
  async deleteItem ({ dispatch }, { payload, key }) {
    ...
    try {
      ...
      const { status, data } = await this.$axios.delete(`/api/programs/${params}`)
  ...
  async addItem ({ dispatch }, { payload }) {
    try {
      const { status, data } = await this.$axios.post(`/api/programs`, payload)
  ...
  async updateItem ({ dispatch }, { payload, id }) {
    try {
      const { status, data } = await this.$axios.put(`/api/programs/${id}`, payload)
