...
export const actions = {
  async queryItems ({ commit, state }) {
    try {
      const { sortBy, descending, page, rowsPerPage, search } = state.pagination
      let queryParams = ''
      ...
      if (search) {
        queryParams += `__+search=${search}`
      }

      const { status, data } = await this.$axios.get(`/api/categories${queryParams}`)
  ...
  async deleteItem ({ dispatch }, { payload, key }) {
    ...
    try {
      ...
      const { status, data } = await this.$axios.delete(`/api/categories/${params}`)
